...
| LG-AUTOMATED | Actual native/backend/extension test suites, fresh/upgrade migrations, contract/property/race/negative tests and CI artifact logs against candidate. No skipped safety tests or empty passing suites. | automated | Engineering; no implementation exists. |
...
| LG-IDENTITY | Entra External ID real tenant/app/redirect/PKCE, two devices, refresh/revoke/sign-out, cross-account denial and production system-browser callback. | automated, manual, live | Identity operator; actual approved identity inputs required. |
...
9. Approve public distribution only after the entire gate ledger passes. Distribution expansion is evidence-driven: inspect error/rollback/usage/privacy indicators after a bounded authorized cohort, then approve further exposure. This is operational risk control, not a product-scope milestone plan.
...
1. Identify impacted app/backend/configuration versions and preserve sanitized logs/evidence. Withdraw the faulty candidate from further offers and stop promotion. Do not delete evidence or pretend already-downloaded copies were recalled.
