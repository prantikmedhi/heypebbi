...
2. User completes identity UI manually. Pebbi accepts only a valid issuer/audience/nonce/state/PKCE return. On valid exchange, store session material in Keychain and register/read back the device; connection becomes `ready` and Home resumes.
...
4. In Settings, user sees current account and device sessions. Revoke names the target; confirmation invalidates it server-side and readback removes or marks it revoked. Revoking this device stops future authenticated calls and locks account data.
...
3. If the request is actionable, create a journaled `queued` task and show its named card; never imply a tool ran merely because the voice said it would. Run PB-015 and applicable approvals.
...
5. Typed follow-ups enter the same conversation and serialized dispatcher. A quiet user can inspect all output and approve using native controls.
...
1. Snapshot origin and verify non-secure target; check selected transcription capability. Start `idle` → `capturing` only after live session readiness, not just an accepted provider configuration.
...
**Recovery:** Focus changed → retain `reviewing` and ask user to refocus/choose. Route failure → `failed` with partial draft, or review completed partial on explicit user choice. Cancel gives `cancelled`; never auto-insert late transcripts. F2/F3; actual outbound send is separate A2. **Acceptance:** [PB-010](../quality/ACCEPTANCE.md).
...
3. Before a context limit, assemble a compaction summary retaining goals, constraints, approved memories, source links, incomplete steps and unresolved approvals. Keep original history accessible; expose the summary and its coverage.
4. A new request reads only the latest permitted memory and summary versions. If compaction fails or critical context is ambiguous, task becomes `waitingForInput` with a concrete clarification, not a loop rejecting all messages.
...
3. A second task for the same Pebbi joins its queue; a different Pebbi may run concurrently under actual capacity. Approved suggestion sources travel with their queue entry.
...
2. `allowOnce` authorizes this version only. Eligible `allowForScope` additionally displays bounded resource/expiry. `deny` dispatches nothing; if essential, enter `waitingForInput` for a revised plan or End task. Voice “yes” works only against one unambiguous current prompt.
...
2. For a new local file within approved scope, generate and persist it atomically. For overwrite, show destination/diff and obtain required approval; preserve existing file if replacement fails.
...
4. After the user reviews the full script/version and referenced inputs and grants fresh `allowOnce` for this run, task runs under declared resource bounds. Capture actual exit status/output, support Stop and do not claim an allowlist is OS sandboxing. Network/external side effects still need appropriate scoped/high-risk approvals.
...
4. Approve or an unambiguous “do it” on the visible proposal atomically stores approval and creates one `queued` task carrying source details/snapshots. Busy Pebbi retains queue entry; repeated clicks reuse it.
...
**Recovery:** Expired source connection explains missing evidence rather than inventing an idea. Changed material or ambiguous voice target requires `waitingForInput`. F0/F4/F7; A1 launch/source consent, A2 for consequential action. **Acceptance:** [PB-024](../quality/ACCEPTANCE.md).
