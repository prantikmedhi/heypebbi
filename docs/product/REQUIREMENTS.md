...
**Approval rules.** A0: local navigation, drafts and explicitly requested reads inside an already authorized scope do not require repetitive confirmation. A1: action outside a granted scope must preview app/connector, operation, resource, relevant data disclosure and consequence; decisions are exactly `allowOnce`, `allowForScope`, `deny`. A2: `allowForScope` is bounded by user, tool class, resource and expiry, is revocable, and never includes payments, credentials, destructive bulk operations, publishing or outbound messages. Those require a fresh exact preview and approval. Protected-effect `allowOnce`/`allowForScope` must originate from deliberate activation of an accessible native approval control showing that exact preview (pointer, keyboard, VoiceOver or equivalent assistive activation); a transcript or model assertion is never consent. A3: credentials, OS permission consent and payment authentication are entered/confirmed by the user; Pebbi never automates their entry or pretends to bypass them. A4: approval is bound to an immutable action version; editing a recipient, amount, destination, command, payload or scope invalidates it. A5: speech such as “do it” may select one visible/current low-risk suggestion, request a task or open a named review; ambiguity becomes `waitingForInput`. Selecting work is not consent to its eventual protected effects, even when a spoken “yes” is unambiguous. A tool allowlist is not an OS sandbox.
...
- **Required:** System-browser Entra External ID OIDC/PKCE sign-in validates the returned identity; Home shows the signed-in account. Device sessions can be listed and revoked. A device ID is public identification, not proof: device-bound calls require the account bearer, matching device ID and separate Keychain-held device credential. Enrollment/re-enrollment requires server-verified recent interactive authentication; revocation invalidates old enrollment authority as well as the device credential, so changing installation/device IDs or refreshing an old bearer cannot restore access. A lost enrollment response requires explicit reauthentication/re-enrollment and retirement of the uncertain enrollment, not retrieval of its secret from a response cache. Sign-out removes session material from Keychain, stops authenticated new work, and locks retained local account data against another account. Explain whether local data remains; deletion is separate.
...
- **States, focus, approvals:** `idle` → `capturing` → `transcribing` → `reviewing` → `inserting` → `completed`; Cancel → `cancelled`; before insertion dispatch it makes zero target edits. If an insertion was already admitted, stop further dispatch and reconcile that one effect; retain inserted/unchanged/unknown outcome in the insertion receipt, not a new lifecycle state. Never automatically undo over user edits, repeat insertion or claim unchanged/completed without evidence. Errors → `failed`, retained transcript available for a new attempt only after any uncertain insertion is reconciled. F2/F3; A0 for confirmed field insertion, A2 for any separate send/execute action.
...
- **Unhappy paths:** Conflicting facts ask which to retain; secrets/sensitive inferences rejected; stale compaction is invalidated after edits; unavailable original source remains marked unavailable. Context construction uses the backend-delivered verified `tokenBudget`, including provider capacity, operator clamps and estimator version; absent/unusable budgeting cannot be guessed from a model name. Changed budgets trigger re-estimation before dispatch. A context-limit rejection invalidates stale budgeting and preserves instructions/evidence for explicit recovery rather than silently deleting them.
...
- **Required:** Independent Pebbis can work concurrently within advertised execution capacity; each Pebbi has one running agent-task attempt and an ordered queue. Independent voice/dictation audio operations do not occupy that queue or count as additional agent attempts; microphone ownership is a separate device-wide limit. A follow-up is atomically assigned to the current attempt at a safe input boundary or becomes a new turn after terminal completion. Preserve message order and acknowledge destination by name. “Retry it” identifies an exact task; ambiguity asks. Queued suggestions retain sources and original approved scope.
...
- **Unhappy paths:** Stale approval cannot execute; failed cancellation never claims rollback; unknown send outcome stays visibly unresolved and cannot auto-retry; unambiguous spoken “yes,” replayed transcripts and model-asserted consent all leave protected actions waiting for native approval; ambiguous task selection asks which task.
...
- **Required:** Each Pebbi has a managed workspace. From an empty workspace, explicitly structured model-authored text is staged using `stageTextArtifact`; ordinary assistant prose never silently becomes a file or executable. Native finalization supplies the immutable artifact ID, exact UTF-8 byte count and digest for a separately authorized create/replace/run action. Staging changes no user target file. Cancel clears unfinished drafts; private drafts/artifacts remain in RAM unless the user explicitly chooses Save, with no private text/hash in the metadata-only journal. Files show origin, producing task, latest modification and verification. External folder access uses explicit chosen scope; resolve paths/symlinks before reads/writes. Preview overwrite/diff and destination. Code execution has declared command, working directory, network/filesystem/resource access, timeout and cancellation. Each run requires full user review of its exact immutable script artifact bytes and inputs plus fresh native `allowOnce`; `scriptArtifactId` and `expectedSha256` bind those reviewed bytes. A scoped grant never makes unreviewed code trusted. Distinguish generated code from executed/tested output.
...
- **States, focus, approvals:** Research task `running` has read-only tools; approved work `queued`; ambiguous voice → `waitingForInput`. F0/F4/F7; A1 for source consent, A5 for low-risk task selection/launch and A1/A2 for protected effects at execution. Spoken selection may queue work or open review but never records protected-effect `allowOnce`/`allowForScope`.
...
- **Required:** Release candidate evidence covers every PB in requirements, flows, screens, acceptance and machine-readable coverage. Exercise real native installation, account, permissions, live models, connector/browser/native control, outputs, billing, recovery, accessibility, update and deletion on authorized environments. Mark each test's environment and actual pass/fail/blocked/not-run result; fixture results cannot satisfy live gates. Final release requires no unresolved safety-critical or mandatory behavior failures. There is one complete-product release authorization after all gates pass, with isolated pre-release verification and continuous post-release monitoring/withdrawal; no required public-cohort promotion sequence.
