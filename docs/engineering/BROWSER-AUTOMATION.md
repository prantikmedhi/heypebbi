...
No Chrome DevTools Protocol server or remote-debugging port is opened on the user's ordinary signed-in profile. No copying a profile/cookies to a headless browser, no invisible parallel signed-in tabs and no raw arbitrary `executeJavaScript` tool. The extension performs a fixed, schema-validated DOM operation set.
...
5. App commits a `pairingId` and identity metadata; popup shows paired app/profile label. Revoke from either end deletes keys, invalidates sessions and removes optional grants on request. No pairing across accounts by accident.
