...
4. **Safety tightens persistent grants and spoken approval.** Vendor notes advertise persistent computer/extra-usage approval and historically more eager writes. Pebbi uses bounded grants, fresh high-risk previews, manual protected UI and explicit CGEvent takeover. Speech may select a low-risk suggestion or open its review, but protected-effect approval must come from the deliberate accessible native control; a transcript, even unambiguous, cannot authorize it. Tool allowlisting is not a sandbox, and cancellation is not rollback. [C](https://www.heyclicky.com/changelog)
